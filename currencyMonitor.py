...
def getCurrencyConversionRates(orig_currency: str, dest_currency: str, api_key: str) -> int:
    ...
    if response.status_code == 200:
        ...
        logger.info(f"Conversion rate from {orig_currency} to {dest_currency}: {conversion_rate}")
...
def main():
    ...
    parser.add_argument('-b', '--base_currency', type=str, default = "USD", help='Base currency to exchange from')
    parser.add_argument('-d', '--dest_currency', type=str, default = "EUR", help='Destination currency to exchange to')
    ...
    parser.add_argument('--sample_rate', type=int, default = "5", help='Sample rate to get the exchange currency values in seconds')
    parser.add_argument('--duration', type=int, default = "60", help='Overall duration for sampling the exchange rate in seconds')
    # Parse the arguments
    ...
    if not average_logger_bool:
        logger.info(f"Average exchange rate over {args.duration} seconds is {np.average(currency_rates)}")
